...
class VKITTI(FlowDataset):
    def __init__(self, aug_params=None, root='/data/fmj/dataset/vkitti_2/', val=False):
        super(VKITTI, self).__init__(aug_params, sparse=True, vkitti=True)
        # if val:
        #     scene_list = ['Scene02']
        # else:
        #     scene_list = ['Scene01', 'Scene06', 'Scene18', 'Scene20']
        scene_list = ['Scene01', 'Scene02', 'Scene06', 'Scene18', 'Scene20']
        for dstype in ['15-deg-left', '15-deg-right', '30-deg-left', '30-deg-right', 'clone', 'fog', 'morning',
                       'overcast', 'rain', 'sunset']:
            for scenetype in scene_list:
                for direction in ['forwardFlow', 'backwardFlow']:
                    image_dirs = sorted(glob(osp.join(root, scenetype, dstype + '/frames/rgb')))
                    image_dirs = sorted([osp.join(f, "Camera_0") for f in image_dirs])
                    flow_dirs = sorted(glob(osp.join(root, scenetype, dstype + '/frames')))
                    flow_dirs = sorted([osp.join(f, direction, "Camera_0") for f in flow_dirs])
                    segmask_dirs = sorted(glob(osp.join(root, scenetype, dstype + '/frames')))
                    segmask_dirs = sorted([osp.join(f, "classSegmentation", "Camera_0") for f in segmask_dirs])
                    for idir, fdir, sdir in zip(image_dirs, flow_dirs, segmask_dirs):
                        images = sorted(glob(osp.join(idir, '*.jpg')))
                        flows = sorted(glob(osp.join(fdir, '*.png')))
                        segmasks = sorted(glob(osp.join(sdir, '*.png')))
                        for i in range(len(flows) - 1):
                            if direction == 'forwardFlow':
                                self.image_list += [[images[i], images[i + 1]]]
                                # self.segmask_list += [[segmasks[i], segmasks[i + 1]]]
                                self.flow_list += [flows[i]]
                            elif direction == 'backwardFlow':
                                self.image_list += [[images[i + 1], images[i]]]
                                # self.segmask_list += [[segmasks[i + 1], segmasks[i]]]
                                self.flow_list += [flows[i]]
...
def fetch_dataloader(args, TRAIN_DS='C+T+K+S+H'):
    ...
    elif args.stage == 'kittic':
        aug_params = {'crop_size': args.image_size, 'min_scale': -0.2, 'max_scale': 0.4, 'do_flip': False}
        kitti15 = KITTI(aug_params, split='training')
        kitti12 = KITTI_12(aug_params, split='training')
        train_dataset = kitti12 + kitti15
    elif args.stage == 'vkitti':
        aug_params = {'crop_size': args.image_size, 'min_scale': -0.2, 'max_scale': 0.4, 'do_flip': False}
        train_dataset = VKITTI(aug_params)
