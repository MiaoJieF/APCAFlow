...
class RelPosEmb(nn.Module):
    ...
    def forward(self, q):
        ...
        # print(batch, heads, h, w, c)
        # print(self.rel_ind[:h, :h].reshape(-1).shape)
...
class Attention(nn.Module):
    ...
    def forward(self, fmap):
        ...
        # torch.Size([1, 128, 32, 64]) torch.Size([1, 128, 32, 64])
        q, k = map(lambda t: rearrange(t, 'b (h d) x y -> b h x y d', h=heads), (q, k))
        # print(q.shape, k.shape)
        # torch.Size([1, 1, 32, 64, 128]) torch.Size([1, 1, 32, 64, 128])
        q = self.scale * q
        if self.args.position_only:
            ...
            # torch.Size([1, 1, 32, 64, 128]) torch.Size([1, 1, 32, 64, 128]) torch.Size([1, 1, 32, 64, 32, 64])
...
class Aggregate(nn.Module):
    ...
    def forward(self, attn, fmap):
        # print(attn.shape, fmap.shape)
        # torch.Size([1, 1, 2048, 2048]) torch.Size([1, 128, 32, 64])
        heads, b, c, h, w = self.heads, *fmap.shape
        # print(heads, b, c, h, w)
        # 1 1 128 32 64
        v = self.to_v(fmap)
